...
class File
{
    // Methods
public:
    static Ptr<File> Create(const String& filename, int mode);
...
private:
    ...
    friend class Ptr<File>;
    friend class Ptr<const File>;
};

inline Ptr<File> File::Create(const String& filename, int mode)
{
